...
namespace AnimeFight
{
	...
	public:
		...
		/*	設定時間點的停留時間上限, Index從1開始, In_Timeoutms設為0時代表需要收到Next CMD才會離開,	若未設定則代表立即經過這個節點.
			若回傳失敗代表Index超過節點數
		*/
		...
		// 進入時間軸(不進入時間軸了話無法使用Next跟ForwardTo)
		void Enter();
	...
	private:
		...
		NodeState GotoNode(std::unique_lock<std::mutex> &unilck, size_t Index);
		...
		std::vector<NodeState>		m_vecTimeNodesLeaveFlag;
